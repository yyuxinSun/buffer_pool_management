...
#define LISTS_FILENAME "lists_1B"
...
#define DATASET "SIFT1000M"

#define N_ENTRIES 1E9
...
auto prepare_queries = [](uint8_t *query_vectors, len_t n_query_vectors, len_t vector_dim, len_t n_results, len_t n_probes)
{
  ...
  for (len_t query_id = 0; query_id < n_query_vectors; query_id++)
  {
    ...
    // for (int i = 0; i < 2; i++) {
        // Query *hot_query = prepare_query(query_vectors, (len_t) k % 50, vector_dim, n_results, n_probes);
...
auto get_recall_at_r = [](const QueryResultsBatch &result_batch, const uint32_t *groundtruth, len_t n_query_vectors, len_t n_results_groundtruth, len_t n_results, len_t r)
{
    ...
    int k = 0;
    for (len_t query_id = 0; query_id < n_query_vectors * 1; query_id++)
    {
        ...
        /** original queries */
        if (query_id % 1 == 0) {
            groundtruth_first_nearest_neighbor = groundtruth[(query_id / 1) * (n_results_groundtruth + 1) + 1];
        }

        /** hot queries */
        if (query_id % 1) {
            groundtruth_first_nearest_neighbor = groundtruth[(k % 50) * (n_results_groundtruth + 1) + 1];
            k++;
        }
    ...
    return (float)n_correct / (n_query_vectors * 1);
};
...
int main() {
    ...
    std::string groundtruth_filepath = join(SIFT_GROUNDTRUTH_DIR, "idx_1000M.ivecs");
    ...
    /** Initialize lists. */
    StorageLists lists;
    if (!file_exists("output/list_archive_1B")) {
        ...
        */
        ...
        std::ofstream ofs("output/list_archive_1B");
        boost::archive::text_oarchive oa(ofs);
        oa << lists;
    } else {
        std::ifstream ifs("output/list_archive_1B");
        boost::archive::text_iarchive ia(ifs);
        ia >> lists;

        FILE *f = fopen("tests/tmp/lists_1B.bin", "r+");
        if (f == nullptr)
        {
        throw std::runtime_error("Could not open file tests/tmp/lists_1B.bin");
        }
        ...
        {
        throw std::runtime_error("Could not mmap file tests/tmp/lists_1B.bin");
        }
        fclose(f);
    }
    ...
    BufferPoolManager* bpm = new BufferPoolManager(100000, &lists, "tests/tmp/lists_1B.bin");
